...
// Sale payload type for API request
...
export default function NewInvoicePage() {
  ...
  useEffect(() => {
    ...
    fields.forEach((item) => {
      newSubtotal += calculateAmount(
        (form.getValues(
          `products.${fields.indexOf(item)}.quantity`
        ) as number) || 0,
  ...
  async function onSubmit(data: InvoiceForm): Promise<void> {
    ...
    try {
      ...
      const saleResponse = await axiosInstance.post<SaleResponse>(
